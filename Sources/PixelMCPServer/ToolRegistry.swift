    ...
    static let uiScreenshot = ToolDefinition(
        ...
        inputSchema: .object([
            ...
            "properties": .object([
                ...
                "som_options": .object([
                    "type": .string("object"),
                    "description": .string("Faz 5 (v0.2.38) + 5b (v0.2.45) + 5c (v0.2.51-54): SoM renderer override — palette, outline_width, badge_size, font_size, text_color, badge_placement, ocr_crop_mode, ocr_min_confidence. Boş bırakırsan default değerler (geri uyumlu eski davranış). badge_placement: 'top_left_inside' (default) | 'top_left_outside' | 'top_right_inside' | 'top_right_outside' | 'smart_corner' | 'label_aware' (AX role bazlı) | 'content_aware' (Vision OCR ile en az çakışan köşe; OCR başarısız ise label_aware fallback). ocr_crop_mode: 'whole_image' (default, tek Vision pass) | 'per_element' (her element için crop edilmiş region'da paralel pass — az element + küçük rect'lerde scoping/locality). ocr_min_confidence: 0.0-1.0 (default 0.0 — tümü kabul). Vision low-confidence noise filter; 0.5 ortalama, 0.8 sıkı. Sadece content_aware iken anlamlı."),
                ]),
            ]),
        ]),
        handler: { params in
